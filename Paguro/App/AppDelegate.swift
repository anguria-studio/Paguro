...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// application that started Paguro.
    ...
    /// Runs each time Paguro becomes the active application. The island uses this
    ...
    /// A regular activation policy only makes Paguro eligible for Command-Tab.
    ...
    /// The moment until which an activation belongs to Paguro's own request.
    ///
    ...
    /// A request that Paguro made for itself expires after this time.
    ///
    ...
    /// Tells whether the current activation follows a request of Paguro itself.
    ...
    /// launch request: at launch the system is already activating Paguro and a
    ...
    /// Marks the activation that Paguro requests for one of its own windows.
    ///
    ...
    /// Activates Paguro and shows its main window after an external action.
    ///
    ...
    /// Tells whether a window drives Paguro's main-window lifecycle.
    ///
    ...
    /// Paguro starts. None of them is a window a person opened, so none of them
    ...
    private func activateInitialWindowIfAvailable() {
        ...
        // `LSUIElement` is NO, so the system already brought Paguro to the front
        // and AppKit already made this window key. Repeating either request
        // hands the front back to the application that launched Paguro: an
    ...
    /// the title "Paguro" but is borderless, and activating it at launch left
    /// the real window behind.
    private var mainWindow: NSWindow? {
        NSApp.windows.first { $0.identifier?.rawValue == "main" }
            ?? NSApp.windows.first {
                Self.carriesMainWindowLifecycle($0) && $0.title == "Paguro"
            }
    }

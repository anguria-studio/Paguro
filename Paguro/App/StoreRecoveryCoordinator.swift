...
final class StoreRecoveryCoordinator {
    ...
    init(
        ...
        case .restoredFromSnapshot(_, let takenAt):
            ...
            banner = Banner(
                message: "Paguro recovered your data from an automatic backup\(when).",
                folderURL: nil,
                isDismissible: true
            )
        case .inMemoryFallback:
            ...
            banner = Banner(
                message: "Your saved data couldn't be loaded, so Paguro is running with temporary storage — changes won't be saved. Paguro keeps automatic backups from before each update; your data folder (with those backups) is at: \(folderURL.path)",
                folderURL: folderURL,
                isDismissible: false
            )
        }

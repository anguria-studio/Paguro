...
///
/// That is not hypothetical. Paguro shared the file with Bartender 6, whose
/// `WidgetSettings` model replaced the spaces and services table for table; the
/// store on disk ended up holding `ZWIDGETSETTINGS` and nothing of Paguro's,
...
/// connection).
///
/// So the store moves to `Application Support/Paguro/default.store`. Debug
/// builds already sit in `Paguro-debug` for the same class of reason and are
/// untouched by this.
///
...
/// be proved to be Paguro's, and nothing is deleted until its copy has been
/// read back.
enum StoreRelocation {
    /// The folder the store moves into, under Application Support.
    static let folderName = "Paguro"
    ...
    static func resolveStoreURL(legacy: URL, scoped: URL) -> URL {
        ...
        // Paguro run in the meantime could have left a store there, and
        ...
        if fm.fileExists(atPath: legacy.path) {
            if StoreInventory.readContent(at: legacy) != nil {
                ...
                // Someone else's file, or one Paguro cannot read. Either way it
                // is not provably ours, so it stays exactly where it is — the
                // backups swept below are what brings the user's data back.
                AppLogger.dataStore.error("The store at the old path is not Paguro's; leaving it and starting from backups")
            }
        }
    ...
    /// Moves every backup Paguro kept beside the legacy store. These are the

...
/// A link whose two ends both resolved.
///
/// `SpaceServiceLink.space` and `.service` are optional in storage, and a link
/// can also outlive a deleted end, so every read site would otherwise repeat
/// the same guard. Resolving once at the fetch boundary makes "both ends are
/// usable" a fact of the type instead of a convention, and lets the rail, the
/// dock, and the stores read `space` and `service` without unwrapping.
struct LiveSpaceServiceLink: Identifiable, Hashable {
    let link: SpaceServiceLink
    let space: Space
    let service: ServiceInstance

    var id: UUID { link.id }
    var sortOrder: Int { link.sortOrder }

    /// Fails when the link, its space, or its service is gone.
    init?(_ link: SpaceServiceLink) {
        guard let ends = link.liveEnds else { return nil }
        self.link = link
        self.space = ends.space
        self.service = ends.service
    }

    static func == (lhs: Self, rhs: Self) -> Bool { lhs.link.id == rhs.link.id }

    func hash(into hasher: inout Hasher) { hasher.combine(link.id) }
}

...
6. Run the Paguro scheme tests in Xcode.

## Sign a local Debug build

An ad-hoc signed app can run most Paguro tests.
...
Do not edit `Paguro.xcodeproj` by hand.
...
- Put pure rules and value types in `PaguroCore`.
...
after the merge. Reviewer approval is optional while the project has one
maintainer; maintainers use the same pull request process.

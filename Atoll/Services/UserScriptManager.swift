...
final class UserScriptManager {
    ...
    func installUserScripts(
        ...
        // while Atoll is in the background, so a service that flips presence to
        ...
        // hold back the notifications Atoll forwards.
    ...
    nonisolated static func makeCSSInjectionScript(css: String) -> String {
        ...
        (function() {
            var CSS = \(literal);
            var existing = document.getElementById('atoll-custom-css');
            if (existing) { existing.textContent = CSS; return; }
            var style = document.createElement('style');
            style.id = 'atoll-custom-css';
    ...
    nonisolated static let callDetectionQueryJS = "window.__atollActiveCall === true"
    ...
    /// firing them, preserving Atoll's `window.Notification` forwarding.
    ...
    /// so a page can't start its idle timer when Atoll loses focus. Pairs with
    ...
    /// Sets `window.__atollActiveCall = true` when a connection is active,
    ...
    private static func makeCallDetectionScript() -> String {
        ...
        (function() {
            ...
            window.__atollActiveCall = false;
            ...
            function updateCallState() {
                ...
                try {
                    ...
                    if (!top.__atollCallFrames) top.__atollCallFrames = {};
                    if (active) {
                        top.__atollCallFrames[frameId] = true;
                    } else {
                        delete top.__atollCallFrames[frameId];
                    }
                    top.__atollActiveCall = Object.keys(top.__atollCallFrames).length > 0;
                } catch (e) {
                    window.__atollActiveCall = active;
                }
            }
    ...
    /// Atoll. **This covers the page-side call only.** A notification raised
    ...
    private func makeNotificationInterceptionScript(serviceID: String) -> String {
        ...
        (function() {
            ...
            if (OrigNotification) {
                var AtollNotification = function(title, options) {
                ...
                AtollNotification.prototype = OrigNotification.prototype;

                // Carry every static across, descriptors and all, before the two
                // overrides below replace the ones Atoll answers itself.
                Object.getOwnPropertyNames(OrigNotification).forEach(function(key) {
                    ...
                    try {
                        ...
                        if (descriptor) Object.defineProperty(AtollNotification, key, descriptor);
                    } catch (e) {}
                });

                Object.defineProperty(AtollNotification, 'permission', {
                    get: function() { return 'granted'; },
                    configurable: true
                });
                AtollNotification.requestPermission = function(cb) {
                    if (cb) cb('granted');
                    return Promise.resolve('granted');
                };

                window.Notification = AtollNotification;
            }

...
enum DarkReaderSupport {
    ...
    @MainActor static var world: WKContentWorld {
        WKContentWorld.world(name: "AtollDarkReader")
    }

    /// Element id for the load cover, so it can be removed when theming is turned
    /// off.
    private static let antiFlashID = "atoll-dr-antiflash"
    ...
    /// `__atollCoverDismiss()` on that world's globals, called from `disableJS`
    ...
    static func antiFlashScript(
        ...
        (function() {
            ...
            window.__atollCoverDismiss = dismiss;
    ...
    if (window.__atollCoverDismiss) { window.__atollCoverDismiss(); }

...
enum AppCapabilities {
    ...
    /// then, passkey prompts fail inside Atoll, so the UI steers users to
    ...
    static let passkeyUnavailableBanner =
        "Passkeys aren't available for sign-in here. Apple doesn't let apps like Atoll use them, so sign in with your password or another method."
}

...
final class ServiceInstance {
    ...
    /// Open a link that leaves this service in an in-app Atoll window instead of
    /// the system browser. Only affects links that no other Atoll service owns —
    ...
    /// Report the page as focused even while Atoll is in the background, so a
    ...
    /// desktop notifications Atoll forwards — an opt-in trade.

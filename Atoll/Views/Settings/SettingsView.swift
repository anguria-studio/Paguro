    ...
    var body: some View {
        Form {
            Section("Service Icons") {
                ...
                Text("Atoll fetches each service's icon from that service's own site. When a site serves none, this asks Google for one, which tells Google the hostname — including a private or self-hosted one. Off by default; services without an icon show their initial instead.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
...
struct AboutSettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section {
                HStack(spacing: 4) {
                    Text("Atoll is based on Chorus by")

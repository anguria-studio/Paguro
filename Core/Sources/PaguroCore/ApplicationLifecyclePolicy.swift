...
/// PaguroCore. The app delegate maps the result to an AppKit activation policy.
...
/// cooperative activation exchange, where the application that launched Paguro
...
/// How Paguro asks for the front when a person acts on it from elsewhere.
public enum ApplicationActivationRequest: Equatable, Sendable {
    /// Paguro already owns the front. Only its window needs to come forward.
    case none
    /// Paguro asks the system for the front.
    case cooperative
    /// Paguro takes the front from the application that owns it.
    case takeFront
}
...
public enum ApplicationLifecyclePolicy {
    ...
    /// window can stay behind the application that started Paguro.
    ///
    ...
    /// A person acting on Paguro from elsewhere must reach a front window.
    ///
    /// This request is not the launch request: no system activation is under
    /// way, so Paguro must name the application that holds the front and take
    ...
    ///
    /// A request that Paguro made for itself must not restore the window. Such
    /// a request always precedes a window that Paguro is about to show, and the

/// What happened when Paguro asked macOS for the notification permission.
///
...
///
/// Paguro must separate two states that look the same from the outside. In
/// both, no banner appears and `notificationSettings()` reports `denied`:
///
/// - the user refused the permission, which Paguro must remember and respect;
/// - macOS never registered Paguro, which Paguro must retry at each start.
///
...
public enum NotificationAuthorizationPolicy {
    /// Tells whether Paguro asks macOS for the permission at this start.
    ///
    /// Paguro asks once for each start unless macOS already permits the
    ...
    /// app that macOS never registered, so Paguro always makes it.
    ...
    /// Merges a fresh permission read into the state that Paguro holds.
    ///
    /// Paguro reads the permission again at each activation, because the user
    ...
    /// Tells whether Paguro keeps this state across a restart.
    ///
    /// Paguro stores no permission of its own. This rule describes the macOS

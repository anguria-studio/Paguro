...
/// BlattaCore. The app delegate maps the result to an AppKit activation policy.
...
/// cooperative activation exchange, where the application that launched Blatta
...
/// How Blatta asks for the front when a person acts on it from elsewhere.
public enum ApplicationActivationRequest: Equatable, Sendable {
    /// Blatta already owns the front. Only its window needs to come forward.
    case none
    /// Blatta asks the system for the front.
    case cooperative
    /// Blatta takes the front from the application that owns it.
    case takeFront
}
...
public enum ApplicationLifecyclePolicy {
    ...
    /// window can stay behind the application that started Blatta.
    ///
    ...
    /// A person acting on Blatta from elsewhere must reach a front window.
    ///
    /// This request is not the launch request: no system activation is under
    /// way, so Blatta must name the application that holds the front and take

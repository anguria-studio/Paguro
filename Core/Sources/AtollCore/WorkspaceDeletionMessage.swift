...
///
/// Atoll deletes a service, and its sign-in data, only when the service
...
public enum WorkspaceDeletionMessage {
    public static func text(orphanedServiceCount: Int) -> String {
        ...
        case 1:
            return "1 service exists only in this workspace. Atoll will delete it and its sign-in data."
        default:
            return "\(orphanedServiceCount) services exist only in this workspace. Atoll will delete them and their sign-in data."
        }
    }
}

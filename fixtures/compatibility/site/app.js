...
const markerKey = "paguro-compatibility-marker";
...
async function sendPageNotification(title = "Paguro fixture", tag = "page-notification") {
...
control("worker-notification").addEventListener("click", async () => {
  ...
  try {
    ...
    await serviceWorkerRegistration.showNotification("Paguro fixture worker call", {
...
control("delayed-notification").addEventListener("click", () => {
  ...
  window.setTimeout(() => {
    sendPageNotification("Delayed Paguro fixture event", "delayed-notification").catch((error) => {
...
control("increment-badge").addEventListener("click", () => {
  unreadCount += 1;
  document.title = `(${unreadCount}) Paguro compatibility fixture`;
  log(`Document title unread count changed to ${unreadCount}.`);
});

control("reset-badge").addEventListener("click", () => {
  unreadCount = 0;
  document.title = "Paguro compatibility fixture";
...
control("simulate-process-failure").addEventListener("click", () => {
  window.location.href = "paguro-fixture://web-content-process-failure";
});

window.addEventListener("message", (event) => {
  if (event.data && event.data.source === "paguro-compatibility-fixture") {

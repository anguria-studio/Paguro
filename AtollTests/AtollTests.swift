...
@MainActor
final class AtollTests: XCTestCase {
    ...
    private func makeFakeStore() throws -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appending(path: "atoll-snapshot-test-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    func testBackupOnlyRunsWhenTheVersionChanges() throws {
        let store = try makeFakeStore()
        defer { try? FileManager.default.removeItem(at: store.deletingLastPathComponent()) }
        let suite = "atoll-snapshot-test-\(UUID().uuidString)"
    ...
    func testVersionedSchemaEntityNamesAreBare() {
        let schemas: [(String, Schema)] = [
            ("V1_5_11", Schema(versionedSchema: AtollSchemaV1_5_11.self)),
            ("V1_5_12", Schema(versionedSchema: AtollSchemaV1_5_12.self)),
            ("VCurrent", Schema(versionedSchema: AtollSchemaVCurrent.self)),
        ]
    ...
    func testMigrationPlanShapeIsContiguousAndIncreasing() {
        let versions = AtollMigrationPlan.schemas.map { $0.versionIdentifier }
        ...
        XCTAssertEqual(
            AtollMigrationPlan.stages.count,
            AtollMigrationPlan.schemas.count - 1,
    ...
    /// `AtollSchemaV…`, bump `AtollSchemaVCurrent`, add a stage + fixture, then
    ...
    func testCurrentStoredShapeIsPinned() throws {
        ...
        XCTAssertEqual(
            ...
            "AppPreferences stored attributes changed — freeze its historical shape before editing (see AtollSchema.swift)"
        )
    }
    ...
    func testFrozenAppPreferencesMatchesLiveModel() {
        let frozen = Schema([AtollSchemaV1_5_11.AppPreferences.self])
    ...
    func testMigratesFrom1_5_11PreservingAllData() throws {
        ...
        try autoreleasepool {
            let schema = Schema(versionedSchema: AtollSchemaV1_5_11.self)
            let config = ModelConfiguration(schema: schema, url: url)
            let container = try ModelContainer(for: schema, configurations: [config])
            let ctx = container.mainContext
            let space = AtollSchemaV1_5_11.Space(id: spaceID, name: "Work", emoji: "🏢", sortOrder: 3)
            space.isMuted = true
            let service = AtollSchemaV1_5_11.ServiceInstance(id: serviceID, label: "Gmail", url: "https://mail.google.com")
            ...
            let link = AtollSchemaV1_5_11.SpaceServiceLink(id: linkID, sortOrder: 5, space: space, service: service)
        ...
        let schema = Schema(versionedSchema: AtollSchemaVCurrent.self)
        let config = ModelConfiguration(schema: schema, url: url)
        let container = try ModelContainer(for: schema, migrationPlan: AtollMigrationPlan.self, configurations: [config])
        let ctx = container.mainContext
    ...
    func testMigratesFrom1_5_12PreservingAllData() throws {
        ...
        try autoreleasepool {
            let schema = Schema(versionedSchema: AtollSchemaV1_5_12.self)
            let config = ModelConfiguration(schema: schema, url: url)
            let container = try ModelContainer(for: schema, configurations: [config])
            let ctx = container.mainContext
            let service = AtollSchemaV1_5_12.ServiceInstance(id: serviceID, label: "Teams", url: "https://teams.microsoft.com")
    ...
    func testMigratesFromPlainSchemaProductionStore() throws {
        ...
        try autoreleasepool {
            let schema = Schema([
                AtollSchemaV1_5_11.ServiceInstance.self,
                AtollSchemaV1_5_11.Space.self,
                AtollSchemaV1_5_11.SpaceServiceLink.self,
                AtollSchemaV1_5_11.AppPreferences.self,
            ])
            let config = ModelConfiguration(schema: schema, url: url)
            let container = try ModelContainer(for: schema, configurations: [config])
            let ctx = container.mainContext
            let space = AtollSchemaV1_5_11.Space(id: spaceID, name: "Work", emoji: "🏢", sortOrder: 1)
            let service = AtollSchemaV1_5_11.ServiceInstance(id: serviceID, label: "Gmail", url: "https://mail.google.com")
            let link = AtollSchemaV1_5_11.SpaceServiceLink(id: UUID(), sortOrder: 0, space: space, service: service)
            let prefs = AtollSchemaV1_5_11.AppPreferences(id: UUID())
    ...
    func testCopyTripleReportsSuccessAndFailure() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("atoll-copytriple-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    /// and the restore only landed when the user next opened Atoll by hand.
    ...
    /// bound Atoll quits anyway and lets AppKit decide.
    ...
    func testRelocationLeavesAnotherAppsStoreAloneAndTakesOnlyTheBackups() throws {
        ...
        XCTAssertNotNil(
            StoreRepair.newestRestorableSnapshot(for: scoped),
            "Atoll's own backups must still move, since they are the only way back"
        )
    }

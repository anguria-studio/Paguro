...
final class NotificationBridgeTests: XCTestCase {
    ...
    func notificationScriptSource() throws -> String {
        ...
        return try XCTUnwrap(
            controller.userScripts.map(\.source).first { $0.contains("blattaNotification") },
    ...
    func testNotificationShimKeepsPrototypeAndStatics() throws {
        ...
        context.setObject(record, forKeyedSubscript: "blattaPost" as NSString)
        context.evaluateScript("""
            window.webkit = { messageHandlers: { blattaNotification: { postMessage: function(m) { blattaPost(m); } } } };
        ...
        XCTAssertEqual(
            ...
            "and Blatta's own overrides must win over the copied ones"
        )
    ...
    /// without this they never reached Blatta.
    @MainActor
    func testNotificationShimForwardsServiceWorkerNotifications() throws {
        ...
        XCTAssertEqual(posted.count, 1, "and the notification must reach Blatta")

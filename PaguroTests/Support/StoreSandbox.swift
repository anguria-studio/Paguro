...
struct StoreSandbox {
    ...
    init(testCase: XCTestCase, label: String, storeName: String = "store.sqlite") throws {
        ...
        let directoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("paguro-\(label)-\(identifier)", isDirectory: true)
        try FileManager.default.createDirectory(
            at: directoryURL,
            withIntermediateDirectories: true
        )

        let suiteName = "PaguroTests.\(label).\(identifier)"
    ...
    static func relocationDirectories(
        ...
        let support = FileManager.default.temporaryDirectory
            .appendingPathComponent("paguro-\(label)-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: support, withIntermediateDirectories: true)
        return (
            support,
            support.appendingPathComponent("default.store"),
            support.appendingPathComponent("Paguro").appendingPathComponent("default.store")
        )
    }
}

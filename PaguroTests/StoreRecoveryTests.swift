...
final class StoreRecoveryTests: XCTestCase {
    ...
    func makeFakeStore() throws -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appending(path: "paguro-snapshot-test-\(UUID().uuidString)")
    ...
    func testBackupOnlyRunsWhenTheVersionChanges() throws {
        let store = try makeFakeStore()
        defer { try? FileManager.default.removeItem(at: store.deletingLastPathComponent()) }
        let suite = "paguro-snapshot-test-\(UUID().uuidString)"
    ...
    func testMigrationPlanShapeIsContiguousAndIncreasing() {
        let versions = PaguroMigrationPlan.schemas.map { $0.versionIdentifier }
        ...
        XCTAssertEqual(
            PaguroMigrationPlan.stages.count,
            PaguroMigrationPlan.schemas.count - 1,
    ...
    /// `PaguroSchemaV…`, bump `PaguroSchemaVCurrent`, add a stage + fixture, then
    ...
    func testCurrentStoredShapeIsPinned() throws {
        ...
        let frozenLink = try XCTUnwrap(
            Schema(versionedSchema: PaguroSchemaV1_5_13.self)
        ...
        XCTAssertEqual(
            ...
            "AppPreferences stored attributes changed — freeze its historical shape before editing (see PaguroSchema.swift)"
        )
    }
    ...
    func testFrozenAppPreferencesMatchesLiveModel() {
        let frozen = Schema([PaguroSchemaV1_5_11.AppPreferences.self])
    ...
    func testMigratesFrom1_5_11PreservingAllData() throws {
        ...
        try autoreleasepool {
            let schema = Schema(versionedSchema: PaguroSchemaV1_5_11.self)
            let config = ModelConfiguration(schema: schema, url: url)
            let container = try ModelContainer(for: schema, configurations: [config])
            let ctx = container.mainContext
            let space = PaguroSchemaV1_5_11.Space(id: spaceID, name: "Work", emoji: "🏢", sortOrder: 3)
            space.isMuted = true
            let service = PaguroSchemaV1_5_11.ServiceInstance(id: serviceID, label: "Gmail", url: "https://mail.google.com")
            ...
            let link = PaguroSchemaV1_5_11.SpaceServiceLink(id: linkID, sortOrder: 5, space: space, service: service)
        ...
        let schema = Schema(versionedSchema: PaguroSchemaVCurrent.self)
        let config = ModelConfiguration(schema: schema, url: url)
        let container = try ModelContainer(for: schema, migrationPlan: PaguroMigrationPlan.self, configurations: [config])
        let ctx = container.mainContext
    ...
    func testMigratesFrom1_5_12PreservingAllData() throws {
        ...
        try autoreleasepool {
            let schema = Schema(versionedSchema: PaguroSchemaV1_5_12.self)
            let config = ModelConfiguration(schema: schema, url: url)
            let container = try ModelContainer(for: schema, configurations: [config])
            let ctx = container.mainContext
            let service = PaguroSchemaV1_5_12.ServiceInstance(id: serviceID, label: "Teams", url: "https://teams.microsoft.com")
    ...
    func testMigratesFromPlainSchemaProductionStore() throws {
        ...
        try autoreleasepool {
            let schema = Schema([
                PaguroSchemaV1_5_11.ServiceInstance.self,
                PaguroSchemaV1_5_11.Space.self,
                PaguroSchemaV1_5_11.SpaceServiceLink.self,
                PaguroSchemaV1_5_11.AppPreferences.self,
            ])
            let config = ModelConfiguration(schema: schema, url: url)
            let container = try ModelContainer(for: schema, configurations: [config])
            let ctx = container.mainContext
            let space = PaguroSchemaV1_5_11.Space(id: spaceID, name: "Work", emoji: "🏢", sortOrder: 1)
            let service = PaguroSchemaV1_5_11.ServiceInstance(id: serviceID, label: "Gmail", url: "https://mail.google.com")
            let link = PaguroSchemaV1_5_11.SpaceServiceLink(id: UUID(), sortOrder: 0, space: space, service: service)
            let prefs = PaguroSchemaV1_5_11.AppPreferences(id: UUID())
    ...
    func testReadContentCountsRowsAndDistinguishesUnknown() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-readcontent-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testReadContentSeesCommittedRowsStillInTheWAL() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-wal-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testCandidateEnumerationCoversAllBackupFamilies() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-inventory-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testCandidatesOrdersBackupsByCompletenessNotFilename() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-order-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testMainFileOnlyWALBackupIsReadableThroughBothReaders() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-walonly-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testApplyPendingRestoreCopiesAndAlwaysBacksUp() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-pending-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testApplyPendingRestoreRevertsWithoutLeavingAForeignWAL() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-revert-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
        defer { try? FileManager.default.removeItem(at: dir) }
        let suite = "paguro-test-\(UUID().uuidString)"
    ...
    func testApplyPendingRestoreMissingSourceLeavesStoreUntouched() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-missing-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
        defer { try? FileManager.default.removeItem(at: dir) }
        let suite = "paguro-test-\(UUID().uuidString)"
    ...
    func testCopyTripleReportsSuccessAndFailure() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-copytriple-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testApplyPendingRestoreReportsFailureWhenTheLiveFileResistsRemoval() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-uchg-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
    ...
    func testApplyPendingRestoreWorksWhenTheLiveStoreCannotBeRead() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-unreadable-live-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
        defer { try? FileManager.default.removeItem(at: dir) }
        let suite = "paguro-test-\(UUID().uuidString)"
    ...
    func testApplyPendingRestoreRefusesWhenTheAsideCannotBeWritten() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-aside-fail-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
        defer {
    ...
    func testApplyPendingRestoreRefusesWhenOnlyTheAsidesWALFailsToCopy() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-aside-partial-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
    ...
    func testEvaluateStoreRecoveryOffersTheFullestBackup() throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-evaluate-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let storeURL = dir.appendingPathComponent("store.sqlite")
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testCoordinatorMapsLaunchOutcomeToBannerAndCleanupSafety() throws {
        ...
        let storeURL = URL(fileURLWithPath: "/tmp/paguro-recovery/default.store")
    ...
    func testCoordinatorReadsTheStoreFileInsteadOfTemporaryContainerDuringFallback() throws {
        let directory = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("paguro-coordinator-\(UUID().uuidString)", isDirectory: true)
    ...
    /// and the restore only landed when the user next opened Paguro by hand.
    ...
    /// bound Paguro quits anyway and lets AppKit decide.
    ...
    /// The ordinary upgrade: Paguro's own store and its backups move into the
    ...
    /// destroyed Paguro's — while Paguro's own backups still come along.
    @MainActor
    func testRelocationLeavesAnotherAppsStoreAloneAndTakesOnlyTheBackups() throws {
        ...
        // Build a Paguro store, snapshot it, then replace the live file with a
        ...
        XCTAssertNotNil(
            StoreRepair.newestRestorableSnapshot(for: scoped),
            "Paguro's own backups must still move, since they are the only way back"
        )
    }
    ...
    /// Once moved, the old path is never read again. An older build of Paguro
    ...
    /// 1.5.13 (the shape Paguro inherited) opens at the current shape with its
    ...
    func testMigratesFrom1_5_13PreservingLinkEnds() throws {
        ...
        try autoreleasepool {
            let schema = Schema(versionedSchema: PaguroSchemaV1_5_13.self)
            let config = ModelConfiguration(schema: schema, url: url)
            let container = try ModelContainer(for: schema, configurations: [config])
            let ctx = container.mainContext
            let space = PaguroSchemaV1_5_13.Space(id: spaceID, name: "Work", emoji: "🏢", sortOrder: 3)
            let service = PaguroSchemaV1_5_13.ServiceInstance(id: serviceID, label: "Slack", url: "https://slack.com")
            service.hibernationPolicyRaw = "never"
            service.hibernateAfterMinutes = 42
            let link = PaguroSchemaV1_5_13.SpaceServiceLink(id: linkID, sortOrder: 7, space: space, service: service)
            ctx.insert(space); ctx.insert(service); ctx.insert(link)
            try ctx.save()
        }
    ...
    /// trapped —
    ///
    ///   Cannot remove Paguro.Space from relationship space on
    ///   Paguro.SpaceServiceLink because an appropriate default value is not
    ///   configured
    ///

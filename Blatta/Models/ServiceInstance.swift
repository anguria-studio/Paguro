...
final class ServiceInstance {
    ...
    /// Open a link that leaves this service in an in-app Blatta window instead of
    /// the system browser. Only affects links that no other Blatta service owns —
    ...
    /// Report the page as focused even while Blatta is in the background, so a
    ...
    /// desktop notifications Blatta forwards — an opt-in trade.

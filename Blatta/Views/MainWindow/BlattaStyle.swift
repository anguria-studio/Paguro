...
///
/// Renewals and MacCleanerNative use the same compact type ramp. Blatta uses the
...
extension Font {
    static let blattaSidebarLabel = Font.system(size: BlattaTypeSize.sidebarLabel)
    static let blattaSidebarLabelSelected = Font.system(
        size: BlattaTypeSize.sidebarLabel,
        weight: .medium
    )
    static let blattaSidebarSection = Font.system(
        size: BlattaTypeSize.sidebarSection,
        weight: .medium
    )
    static let blattaSidebarAccessory = Font.system(
        size: BlattaTypeSize.sidebarAccessory,
        weight: .medium
    ).monospacedDigit()
    static let blattaToolbarControl = Font.system(
        size: BlattaTypeSize.toolbarControl,
        weight: .medium
    )
    static let blattaToolbarTitle = Font.system(
        size: BlattaTypeSize.toolbarTitle,
        weight: .medium
    )
    static let blattaBody = Font.system(size: BlattaTypeSize.body)
}
...
enum SidebarPresentation: Equatable {
    ...
    func width(iconRailBaseSize: Double) -> CGFloat {
        switch self {
        case .expanded: BlattaMetric.Sidebar.expandedWidth
        case .collapsed: BlattaMetric.Sidebar.collapsedWidth(iconSize: iconRailBaseSize)
        }
    }

    var serviceRowHeight: CGFloat {
        switch self {
        case .expanded: BlattaMetric.Sidebar.rowHeight
        case .collapsed: BlattaMetric.Sidebar.dockRowHeight
        }
    }
    ...
    var surfaceTopInset: CGFloat {
        switch self {
        case .expanded: BlattaMetric.Sidebar.surfaceInset
        case .collapsed: BlattaMetric.Sidebar.collapsedSurfaceTopInset
        }
    }

    var contentTopInset: CGFloat {
        switch self {
        case .expanded: BlattaMetric.Sidebar.topBarHeight
        case .collapsed: BlattaMetric.Sidebar.collapsedContentTopInset
        }
    }
    ...
    var toggleCenterX: CGFloat {
        switch self {
        case .expanded:
            BlattaMetric.Sidebar.expandedWidth
                - BlattaMetric.Sidebar.expandedToggleTrailingInset
                - (BlattaMetric.Toolbar.sidebarToggleSize / 2)
        case .collapsed:
            BlattaMetric.Sidebar.collapsedWidth
                + BlattaMetric.Toolbar.collapsedLeadingInset
                + (BlattaMetric.Toolbar.sidebarToggleSize / 2)
        }
    }
...
enum BlattaColor {
    ...
    enum Text {
        ...
        static let secondary = BlattaColor.ink(light: 0.60, dark: 0.62)
        static let tertiary = BlattaColor.ink(light: 0.55, dark: 0.52)
        static let selectedOnGlass = BlattaColor.dynamic(
            light: .black,
            dark: .white
        )
    }
    ...
    enum Fill {
        ...
        static let quietSurface = BlattaColor.dynamic(
            light: .white.withAlphaComponent(0.85),
            dark: .white.withAlphaComponent(0.055)
        )
        /// A protective tint over Blatta-owned transient material surfaces. It
...
/// Applies the shell-wide tint scale to a Blatta-owned material surface.
///
...
private struct BlattaMaterialBackgroundModifier: ViewModifier {
...
extension View {
    /// Gives a Blatta-owned surface native material and the shared shell tint.
    func blattaMaterialBackground(_ material: Material) -> some View {
        modifier(BlattaMaterialBackgroundModifier(material: material))
    }
}
...
struct RowMark: Equatable {
    ...
    var fillStyle: AnyShapeStyle {
        switch fill {
        case .selected: return AnyShapeStyle(BlattaColor.Fill.sidebarSelection)
        case .hover: return AnyShapeStyle(BlattaColor.Fill.rowHover)
...
struct BlattaToolbarButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .font(.system(size: BlattaMetric.Toolbar.glyphSize, weight: .medium))
            .frame(
                width: BlattaMetric.Toolbar.controlSize,
                height: BlattaMetric.Toolbar.controlSize
            )
            .background(
                isHovering && isEnabled
                    ? BlattaColor.Fill.controlHover
                    : (isSelected ? BlattaColor.Fill.control : Color.clear),
                in: Circle()
            )
            .contentShape(Circle())
...
struct BlattaSidebarButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .font(.system(size: BlattaMetric.Toolbar.sidebarGlyphSize, weight: .medium))
            .frame(
            ...
            .background {
                if isCollapsed {
                    ...
                    if isHovering {
                        Circle()
                            .fill(BlattaColor.Fill.controlHover)
                    }
                } else if isHovering {
                    Circle()
                        .fill(BlattaColor.Fill.controlHover)

...
struct WebContentView: View {
        ...
        VStack(spacing: 0) {
            if appState.railLayout == .sidebar {
                WebContentHeader(
                    ...
                    title: selectedService?.label ?? "Blatta",
                    reservesSidebarToggleSpace: sidebarIsCollapsed,
                    sidebarToggleLeadingInset: BlattaMetric.Toolbar.collapsedLeadingInset(
    ...
    private func loadWebViewForSelectedService() {
        // Set the native web appearance before Blatta creates a new WKWebView.
        ...
        // Apply the effective zoom (per-service if set, else the Blatta-wide
    ...
    /// Blatta's web views. Auto-hides after a short delay; the "seen" state is
    ...
    private var passkeyNoticeBanner: some View {
        ...
        .background(BlattaColor.Fill.quietSurface)

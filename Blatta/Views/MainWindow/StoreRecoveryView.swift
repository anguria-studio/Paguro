...
/// Lists every store Blatta could put back: the live one and each backup it
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Restore your workspaces and services")
                .font(.headline)
            Text("Blatta keeps a copy of your data before each update. Pick the one you want and Blatta will restart to put it back. Your current data is set aside first, so nothing is thrown away.")
                .font(.callout)
            ...
            HStack {
                ...
                Button("Restore and Restart") {
                    ...
                    guard appState.storeRecovery.chooseRestore(selectedCandidate) else {
                        failureMessage = "Blatta could not restart itself. Quit and open it again to put this backup back."
                        return
                    }

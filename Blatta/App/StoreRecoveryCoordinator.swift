...
final class StoreRecoveryCoordinator {
    ...
    init(
        ...
        case .restoredFromSnapshot(_, let takenAt):
            ...
            banner = Banner(
                message: "Blatta recovered your data from an automatic backup\(when).",
                folderURL: nil,
                isDismissible: true
            )
        case .inMemoryFallback:
            ...
            banner = Banner(
                message: "Your saved data couldn't be loaded, so Blatta is running with temporary storage — changes won't be saved. Blatta keeps automatic backups from before each update; your data folder (with those backups) is at: \(folderURL.path)",
                folderURL: folderURL,
                isDismissible: false
            )
        }

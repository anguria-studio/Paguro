...
final class AppState {
    ...
    /// Blatta-wide default page zoom, applied to services without an explicit
    ...
    init(
        ...
        // `StoreLoader` opens it through `BlattaMigrationPlan`, so an older
        // store migrates through named, tested stages instead of inference. See
        // `Blatta/Models/Schema/BlattaSchema.swift`.
        ...
        #if DEBUG
        let debugDir = URL.applicationSupportDirectory.appending(path: "Blatta-debug")
    ...
    /// which prefers switching to a matching Blatta service over opening
    ...
    /// and which targets a different origin. If any other Blatta service owns
    ...
    private func handleExternalLink(_ url: URL, from sourceServiceID: UUID?) {
        ...
        // No Blatta service owns this link. Open it in an in-app window when the
    ...
    /// Saves a new Blatta-wide default zoom and applies it to every open service
    ...
    func authenticate() {
        ...
        context.evaluatePolicy(.deviceOwnerAuthentication, localizedReason: "Unlock Blatta") { [weak self] success, error in
            Task { @MainActor in
    ...
    /// Counts the services that Blatta deletes together with this space.
    ...
    private func preloadActiveSpaceServices() {
        ...
        // posts notification banners through the `blattaNotification` handler on

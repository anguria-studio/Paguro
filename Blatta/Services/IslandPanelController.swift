...
/// The shared Blatta appearance values that apply to the island surface.
...
final class IslandPanelController {
    ...
    /// Blatta, so the island holds every render until the launch settles.
    ...
    ///
    /// The user reads that conversation in Blatta, so those events are no longer
...
private struct NotificationIslandPanelView: View {
    ...
    private var expandedContent: some View {
        ...
        .accessibilityLabel("Blatta recent notifications")
    }
    ...
    private var compactAccessibilityLabel: String {
        guard let content = model.content else {
            return "Blatta notification island"
        }
        if let body = content.event.body {

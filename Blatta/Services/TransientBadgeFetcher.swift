...
final class TransientBadgeFetcher {
        ...
        while true {
            ...
            if let js = target.badgeJS {
                if let result = try? await webView.evaluateJavaScript(js),
                   let count = BadgeCountExtractor.readJSResult(result).count {
                    // The Core rule reads an NSNumber built from a double and
                    // element text such as "9+" or "1,234". A nil count means
                    // the expression could not read its page, which must not
                    // count as a reading of zero.
                    best = max(best, count)
                }
    ...
    /// count), and content blockers. It OMITS the `blattaNotification` handler on
